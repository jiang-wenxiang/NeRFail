...
from model.GaussNet import gauss_net
...
# other config
expname = scene_name + "_attack_NeRFail_S"
...
test_img_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/{}'.format("test")
train_img_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/{}'.format("train")
val_img_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/{}'.format("val")

test_img_mask_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/attack_masks/{}'.format("test")
train_img_mask_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/attack_masks/{}'.format("train")
val_img_mask_save_to_dir = 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/attack_masks/{}'.format("val")

change_target_path = os.path.join(change_target_base, dataname, 'test')
save_target_path = os.path.join(basedir, dataname, 'NeRFail_S_'+str(base_mask_image_number)+'P_'+str(attack_epochs)+'_to_'+str(attack_target_label_int)+'_e_'+str(epsilon)+'_a_'+str(a)+'/attack_masks/', 'base_masks')
...
if scene_name == "lego" and base_mask_image_number == 2:
    ...
    change_target_img_index_list = [50, 75, 100, 125]
elif scene_name == "materials" and base_mask_image_number == 4:
    change_target_img_index_list = [0, 50, 75, 125]
...
if model_name == "my_model":
    ...
    model = alexnet(num_classes=num_classes, init_weights=False)
...
change_target_tensor_init = change_target_tensor.clone().detach()
change_target_tensor_best = change_target_tensor.clone().detach()

optimizer = optim.SGD([change_target_tensor], lr=0.01, momentum=0.9)
...
for epoch in range(attack_epochs):
    ...
    for index, ori_img, img_index_and_weight, img_save_to_name, img_mask_save_to_name in tqdm.tqdm(now_dataloader):
        ...
        # attack forward
        x, r, cla, ori_img, ori_cla = net(change_target_tensor, img_index_and_weight, ori_img, zero_init_mask)
        ...
        if epoch < (attack_epochs - 1):
            ...
            # attack
            if targeted_attack:
                change_target_tensor_rgba = change_target_tensor - a * torch.sign(change_target_tensor.grad.data).to(device)
                # if zero_init_mask:
            ...
            z_change_target_tensor_alpha = change_target_tensor_alpha + 0.001
            change_target_tensor_rgb_max = change_target_tensor_init[:, :, :, :3] + epsilon
            change_target_tensor_rgb_min = change_target_tensor_init[:, :, :, :3] - epsilon

            temp = torch.cat([change_target_tensor[:, :, :, :3].unsqueeze(0), change_target_tensor_rgb_min.unsqueeze(0)], 0)
            change_target_tensor = torch.cat([torch.max(temp, dim=0)[0], change_target_tensor[:, :, :, 3].unsqueeze(-1)], dim=-1)

            temp = torch.cat([change_target_tensor[:, :, :, :3].unsqueeze(0), change_target_tensor_rgb_max.unsqueeze(0)], 0)
            change_target_tensor = torch.cat([torch.min(temp, dim=0)[0], change_target_tensor[:, :, :, 3].unsqueeze(-1)], dim=-1)
    ...
    beta_attack_epoch_loss = beta_attack_loss / len(now_dataloader.dataset)
    attack_epoch_acc = attack_corrects.double() / len(now_dataloader.dataset)
    attack_epoch_img_loss = attack_img_loss / len(now_dataloader.dataset)
    beta_attack_epoch_img_loss = beta_attack_img_loss / len(now_dataloader.dataset)
    attack_epoch_total_loss = attack_total_loss / len(now_dataloader.dataset)
...
test_for_inception(print_e=epsilon, model_name=model_name, base_mask_image_number=base_mask_image_number,
                   scene_name=scene_name, target_class_idx=attack_target_label_int, attack_epochs=attack_epochs,
                   method_name='NeRFail_S', setname="test", step=0)

test_for_inception(print_e=epsilon, model_name=model_name, base_mask_image_number=base_mask_image_number,
                   scene_name=scene_name, target_class_idx=attack_target_label_int, attack_epochs=attack_epochs,
                   method_name='NeRFail_S', setname="val", step=0)
