...
def render(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'pts_max']
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None,
                savedir=None, render_factor=0, only_render_img=True):
    H, W, focal = hwf
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, pts_max, _ = render(H, W, K, chunk=chunk, c2w=c2w[:3, :4], **render_kwargs)
        rgbs.append(rgb.cpu().numpy())
        disps.append(disp.cpu().numpy())
        ptss.append(pts_max.cpu().numpy())
        if i == 0:
            print(rgb.shape, disp.shape, pts_max.shape)
        ...
        if savedir is not None:
            ...
            if not only_render_img:
                npy_filename = os.path.join(savedir, '{:03d}.npy'.format(i))
                np.save(npy_filename, ptss[-1])
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = torch.cat([dists, torch.Tensor([1e10]).to(dists.device).expand(dists[..., :1].shape)],
                      -1)  # [N_rays, N_samples]
...
def render_rays(ray_batch,
    ...
    weight_max_indexs = torch.argmax(weights, dim=1)

    pts_shape = pts.shape
    pts_max = pts[range(pts_shape[0]), weight_max_indexs]

    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'pts_max': pts_max}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def config_parser():
    ...
    parser.add_argument("--datadir", type=str, default='./data/corner',
    ...
    parser.add_argument("--render_val", action='store_true', default=True,
                        help='render the val set instead of render_poses path')
    ...
    parser.add_argument("--only_render_img", type=bool, default=True)
